...
export default function ImoveisAluguelPage({ params }: { params: Promise<{ corretor: string }> }) {
  ...
  const items = all.filter((p) => p.rent);
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      >
        ...
        <h1 className="font-medium text-3xl md:text-5xl text-white mt-6 leading-snug">Imóveis para aluguel</h1>
        <p className="text-white text-lg md:text-xl mt-2">Todos os imóveis disponíveis para locação.</p>
      </div>
      ...
      <section className="py-12">
        <div className="container mx-auto px-4">
          {items.length > 0 ? (
            ...
            <p className="text-[#777777]">Nenhum imóvel para aluguel.</p>
          )}
        </div>
      </section>
    </div>
