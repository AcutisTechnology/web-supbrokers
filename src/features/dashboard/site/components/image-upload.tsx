...
async function uploadBrandImage(file: File): Promise<string> {
  ...
  const response = await api
    .post('upload/brand-image', { body: formData, timeout: 30000 })
    .json<{ fileUrl: string }>();
  ...
  const uploadImage = async (file: File) => {
    ...
    try {
      ...
      progressInterval = setInterval(() => {
        setUploadProgress((prev) => (prev >= 90 ? 90 : prev + Math.floor(Math.random() * 5) + 1));
      }, 300);
      ...
      onChange(imageUrl);
      toast.success('Imagem enviada com sucesso! Clique em "Salvar alterações" para aplicar as mudanças.');
    } catch (error) {
      if (progressInterval) clearInterval(progressInterval);
      ...
      if (error instanceof Error) {
        if (error.name === 'TimeoutError' || error.message.includes('timeout')) {
          toast.error('O servidor está demorando para responder. Tente novamente.');
        } else {
          ...
          <div className="relative group">
            <div className="relative w-full h-48 bg-gray-50 flex items-center justify-center p-4">
              <div className="relative w-full h-full flex items-center justify-center">
                <Image src={value} alt="Logo da marca" fill className="object-contain" unoptimized />
              </div>
