  ...
  return (
    <Card className="p-6 shadow-sm mb-6">
      <div className="mb-6 flex items-center gap-2">
        ...
        <span className="font-semibold text-[#1c1b1f] text-lg">Demonstrativo de pagamentos</span>
      </div>
      <div className="overflow-x-auto">
        <table className="w-full">
          <thead>
            <tr className="border-b border-[#e0e0e0]">
              <th className="text-left py-3 px-2 text-sm font-medium text-[#969696]">Período</th>
              <th className="text-left py-3 px-2 text-sm font-medium text-[#969696]">Vencimento</th>
              <th className="text-left py-3 px-2 text-sm font-medium text-[#969696]">Pagamento</th>
              <th className="text-right py-3 px-2 text-sm font-medium text-[#969696]">Aluguel</th>
              <th className="text-right py-3 px-2 text-sm font-medium text-[#969696]">Condomínio</th>
              <th className="text-right py-3 px-2 text-sm font-medium text-[#969696]">IPTU</th>
              <th className="text-right py-3 px-2 text-sm font-medium text-[#969696]">Total</th>
              <th className="text-center py-3 px-2 text-sm font-medium text-[#969696]">Status</th>
            </tr>
          </thead>
          <tbody>
            {pagamentos.map((pagamento) => (
              <tr key={pagamento.id} className="border-b border-[#f0f0f0] hover:bg-[#f9f9f9] transition-colors">
                <td className="py-4 px-2">
                  <div className="font-medium text-[#1c1b1f] text-sm">{pagamento.mes}</div>
                </td>
                <td className="py-4 px-2">
                  <div className="text-sm text-[#969696]">{formatDateBR(pagamento.dataVencimento)}</div>
                </td>
                <td className="py-4 px-2">
                  <div className="text-sm text-[#969696]">{formatDateBR(pagamento.dataPagamento)}</div>
                ...
                <td className="py-4 px-2 text-right">
                  <div className="text-sm font-semibold text-[#16ae4f]">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mt-6 pt-6 border-t border-[#e0e0e0]">
        <div className="bg-[#16ae4f]/5 rounded-lg p-4">
          ...
          <div className="text-lg font-semibold text-[#16ae4f]">
            R$ {(pagamentos.reduce((acc, p) => acc + p.total, 0)).toLocaleString('pt-BR', { minimumFractionDigits: 2 })}
          </div>
        </div>
        ...
        <div className="bg-[#9747ff]/5 rounded-lg p-4">
          ...
          <div className="text-lg font-semibold text-[#9747ff]">
            {Math.round(pagamentos.reduce((acc, p) => acc + p.diasAtraso, 0) / pagamentos.length)} dias
          </div>
        </div>
